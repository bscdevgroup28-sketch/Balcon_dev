...
router.post('/', authEnhanced_1.authenticateToken, (0, authEnhanced_1.requireRole)(['admin', 'owner', 'office_manager']), (0, validation_1.validate)({ body: validation_2.createOrderSchema }), async (req, res) => {
    try {
        ...
        // Verify user exists
        const user = await models_1.User.findByPk(orderData.userId);
        if (!user) {
            return res.status(404).json({
                success: false,
...
router.put('/:id', authEnhanced_1.authenticateToken, (0, authEnhanced_1.requireRole)(['admin', 'owner', 'office_manager']), (0, validation_1.validate)({ params: validation_2.idParamSchema, body: validation_2.updateOrderSchema }), async (req, res) => {
    try {
        const { id } = req.validatedParams;
...
router.delete('/:id', authEnhanced_1.authenticateToken, (0, authEnhanced_1.requireRole)(['admin', 'owner']), (0, validation_1.validate)({ params: validation_2.idParamSchema }), async (req, res) => {
    try {
        const { id } = req.validatedParams;
        const order = await models_1.Order.findByPk(id);
        if (!order) {
