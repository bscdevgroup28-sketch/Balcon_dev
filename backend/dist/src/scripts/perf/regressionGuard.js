...
function loadGolden() {
    if (!USE_GOLDEN)
        return null;
    try {
        const file = path_1.default.join(process.cwd(), 'perf-history', 'golden-baseline.json');
...
function evaluate() {
    ...
    for (const [scenario, list] of Object.entries(hist)) {
        ...
        if (golden && golden[scenario]) {
            ...
            if (gP50 !== null && p50Delta !== null)
                p50Delta = Math.max(p50Delta, gP50);
            else if (gP50 !== null)
                p50Delta = gP50;
            if (gP95 !== null && p95Delta !== null)
                p95Delta = Math.max(p95Delta, gP95);
            else if (gP95 !== null)
                p95Delta = gP95;
            if (gRps !== null && rpsAvgDelta !== null)
                rpsAvgDelta = Math.min(rpsAvgDelta, gRps); // pick worse (more negative)
            else if (gRps !== null)
                rpsAvgDelta = gRps;
        }
        const violations = [];
        if (p50Delta !== null && p50Delta > MAX_P50_INCREASE_PCT)
            violations.push(`p50 increased ${p50Delta.toFixed(2)}% > ${MAX_P50_INCREASE_PCT}%`);
        if (p95Delta !== null && p95Delta > MAX_P95_INCREASE_PCT)
            violations.push(`p95 increased ${p95Delta.toFixed(2)}% > ${MAX_P95_INCREASE_PCT}%`);
        if (rpsAvgDelta !== null && rpsAvgDelta < 0 && Math.abs(rpsAvgDelta) > MAX_RPS_DROP_PCT)
            violations.push(`rpsAvg dropped ${Math.abs(rpsAvgDelta).toFixed(2)}% > ${MAX_RPS_DROP_PCT}%`);
...
(function main() {
    ...
    if (process.env.PERF_MD || process.env.GITHUB_STEP_SUMMARY) {
        ...
        try {
            const summaryPath = process.env.GITHUB_STEP_SUMMARY || path_1.default.join(process.cwd(), 'perf-history', 'regression-summary.md');
            if (!process.env.GITHUB_STEP_SUMMARY) {
                const dir = path_1.default.join(process.cwd(), 'perf-history');
                if (!fs_1.default.existsSync(dir))
                    fs_1.default.mkdirSync(dir, { recursive: true });
            }
            fs_1.default.appendFileSync(summaryPath, md + '\n');
        }
        catch { /* ignore */ }
    }
    if (failed) {
