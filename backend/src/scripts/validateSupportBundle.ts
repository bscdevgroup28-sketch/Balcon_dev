...
function main() {
  const proc = spawnSync('node', ['dist/src/scripts/supportBundle.js'], { encoding: 'utf8' });
  if (proc.status !== 0) {
  ...
  let json: any;
  try { json = JSON.parse(proc.stdout.split('\n').slice(-jsonLines(proc.stdout)).join('\n')); } catch {
    try { json = JSON.parse(proc.stdout); } catch (e) {
...
function jsonLines(out: string) { return Math.min(400, out.split('\n').length); }
