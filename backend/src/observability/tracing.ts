...
export function initTracingIfEnabled() {
  ...
  try {
    ...
    if (process.env.OTEL_DEBUG === '1') diag.setLogger(new DiagConsoleLogger(), DiagLogLevel.INFO);
    ...
    sdk.start().then(()=> logger.info('[tracing] OpenTelemetry SDK started')).catch((e: any)=> logger.warn('[tracing] SDK start failed', e));
    ...
    try {
      ...
      sequelizeModule.Sequelize.prototype.query = async function patchedQuery(this: any, sql: any, options: any) {
    ...
    logger.warn('[tracing] OpenTelemetry not initialized (modules missing?)');
  }
}

function incrementMetricSafe(name: string) {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
...
export function recordHttpSpanResult(error: boolean) {
  incrementMetricSafe('tracing.spans.http.server.total');
  if (error) incrementMetricSafe('tracing.spans.http.server.error');
}
