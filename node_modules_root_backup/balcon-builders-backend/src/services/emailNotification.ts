...
export class EmailNotificationService {
  private isProduction = config.server.nodeEnv === 'production';
  private sendGridApiKey = config.email?.sendgridApiKey;
  ...
  private async sendEmail(to: string, template: EmailTemplate): Promise<boolean> {
    try {
      if (!this.isProduction || !this.sendGridApiKey) {
        ...
        logger.info(`Content: ${template.textContent}`);
   ...
   */
  private generateInquiryAssignedTemplate(data: NotificationData): EmailTemplate {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <p><a href="${config.server.baseUrl}/projects/${project.id}" style="background-color: #2c5aa0; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View Inquiry Details</a></p>
   ...
   */
  private generateNewInquiryTemplate(data: NotificationData): EmailTemplate {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <p><a href="${config.server.baseUrl}/admin/projects/${project.id}" style="background-color: #2c5aa0; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Review and Assign</a></p>
  </div>
</body>
</html>
    `.trim();
   ...
   */
  async notifyNewInquiry(project: Project, customer: User): Promise<boolean> {
    try {
      ...
      const results = await Promise.all(
        admins.map(admin => this.sendEmail(admin.email, template))
      );

      const successCount = results.filter(result => result).length;
      logger.info(`New inquiry notification sent to ${successCount}/${admins.length} admins`);

      return successCount > 0;

    } catch (error) {
      logger.error('Error sending new inquiry notifications:', error);
      return false;
    }
  }
   ...
   */
  async notifyInquiryAssigned(project: Project, salesRep: User, customer: User): Promise<boolean> {
    try {
      ...
      if (success) {
        logger.info(`Inquiry assignment notification sent to ${salesRep.email}`);
      }

      return success;

    } catch (error) {
      logger.error('Error sending inquiry assignment notification:', error);
      return false;
    }
  }
   ...
   */
  async notifyStatusChange(project: Project, customer: User, oldStatus: string, newStatus: string): Promise<boolean> {
    try {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <p><a href="${config.server.baseUrl}/projects/${project.id}" style="background-color: #2c5aa0; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View Project Details</a></p>
    ...
    <p style="color: #666; font-size: 12px;">
      ...
      if (success) {
        logger.info(`Status change notification sent to ${customer.email} for project ${project.inquiryNumber}`);
      }
